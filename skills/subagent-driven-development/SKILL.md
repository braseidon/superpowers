...
- **A partial return (turn cap) is not DONE** and is past the gate: re-brief a fresh implementer with the brief path, the report file and "check `git status` first; a prior implementer's edits are on disk, usually mid-edit: typecheck them before trusting them, judge them on merit", or bump. One bounded resume is allowed first: "commit what is green and write the report; no new edits". A turn is one assistant message; the agent cannot see its count, so a brief never says "budget your turns" (split the scope instead).
...
- Record the implementer's agent identity and the token count from its Agent result — fix rounds 1-3 resume it when it passes the resume gate.
