...
- Do not write the plan inline, whatever the session model: dispatch a fresh Opus subagent at maximum reasoning effort (effort-pinned agent type like `general-xhigh` if available) that invokes superpowers:writing-plans with the spec path, saves + commits the plan + `.tasks.json`, and returns the plan path WITHOUT running the Execution Handoff (subagents cannot AskUserQuestion). A fresh writer working from the spec alone is the test that the spec stands on its own — a wall it hits is a spec gap surfacing at the cheap moment. In-session plan writing only on explicit user grant.
