...
On the architectural path, a spec the user wants changed goes back to
"Write design doc" and through self-review again before the next gate.
...
- Do not write the plan inline, whatever the session model: dispatch a fresh Opus subagent at xhigh reasoning effort (the project's `plan-writer` agent if it exists, else an effort-pinned agent type like `general-xhigh`) that invokes superpowers:writing-plans with the spec path, saves + commits the plan + `.tasks.json`, and returns the plan path WITHOUT running the Execution Handoff (subagents cannot AskUserQuestion). A fresh writer working from the spec alone is the test that the spec stands on its own — a wall it hits is a spec gap surfacing at the cheap moment. In-session plan writing only on explicit user grant.
- **Review loop:** run the project's independent doc review on the returned plan. Findings go back to the SAME writer — resume it (e.g. SendMessage) with the review path; it applies findings, re-runs its self-checks, commits, and returns. Re-review (delta round) until approved. Never apply review findings yourself — coordinator fixes skip the writer's self-checks and bloat coordinator context. If the harness cannot resume a subagent, dispatch a fresh writer carrying spec + plan + review paths. Then adjudicate any `[FABLE-ADJUDICATE]` markers and run the Execution Handoff yourself.
