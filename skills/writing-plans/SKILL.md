...
**11. Tier vocabulary and spread (mechanical):** `grep -oE 'modelTier\\?": *\\?"[a-z]+' <plan>.tasks.json | grep -oE '[a-z]+$' | sort | uniq -c` (the fence sits JSON-escaped inside `description`, so a grep for a bare `"modelTier"` matches nothing and passes a broken file) — only `mechanical`, `standard`, `frontier` may appear, and the counts sum to the task count (`grep -c '"subject"'`). Any other word (`judgment`, `orchestrator`, prose) is a plan failure: the routing gates reject it and the controller falls back to guessing. Then read the spread: `frontier` above a third of the tasks in a plan whose steps carry the code means the tier test was not applied — re-tier each `frontier` task by the one question (does a decision remain at edit time?) before committing.

**12. Dispatch waves:** Confirm the `## Dispatch waves` section exists, every task appears in exactly one wave, wave 1 is non-empty, and every `blockedBy` edge names something consumed (walk the edges: if you cannot say what artifact of the blocker the blocked task reads, delete the edge). **Then check every edge against the wave list: a `blockedBy` edge forces the blocked task into a strictly LATER wave than every blocker, never a later slot in the same one.** A wave is a parallel dispatch, so "runs last within its wave" is not something the schedule can express — adding an edge without moving the task leaves it racing the blocker it was supposed to follow. Adding edges late is where this breaks: the edge is the easy half, the renumbering below it is the half that gets skipped.
...
Internal reference (do NOT show): README sections `#force-re-validation-on-user-thrown-gate-close` + `#re-validate-gates-on-plan-complete-claims` in `~/.claude/plugins/marketplaces/braseidon-superpowers/README.md`. Hooks: `hooks/examples/{post-task-complete-revalidate,stop-revalidate-user-gates}.sh`. Design doc: `docs/user-gate-flow.md`.
...
> Invoke superpowers:executing-plans for `<plan path>`. The plan author session "`<this session's title>`" is still running — on any ambiguity or design question, find it with ListAgents and ask it via SendMessage before guessing.
...
- `frontier` — the steps leave a design choice open, or the task needs broad codebase understanding the brief cannot carry. Name which in the task's prose, one clause. A skill-gated or hard-trigger surface is NOT a reason: it makes the skill a brief requirement (name it in the task's Files or Context so the implementer loads it), and the tier still comes from whether a decision remains once the steps carry the code. A plan whose steps carry the code and still tiers most tasks `frontier` has mis-tiered them.
